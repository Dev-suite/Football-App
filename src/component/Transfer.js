    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            Click on the players to see their transfer status
          </p>
        </div>

        {state &&
          state.slice(0, loader).map((player, index) => {
            return (
              <Player
                key={index}
                id={player.player.id}
...
function MyVerticallyCenteredModal(props) {
  ...
  return (
    <div>
      ...
      <p>
        <span className="p-3 fw-bold">Name: {props.name}</span>
        <br />
        <span className="p-3 fw-bold">Age: {props.age}</span>
        <br />
        <span className="p-3 fw-bold">Nationality: {props.nationality}</span>
        <br />
        {transfer && (
          <>
            <span className="text-danger p-3 fw-bold">
              OUT ⬇️:{transfer.teams.out.name}{" "}
            </span>
            <span className="text-primary p-3 fw-bold">
              IN ⬆️: {transfer.teams.in.name}
            </span>
            <span className="text-secondary p-3 fw-bold">
              type: {transfer.type}
            </span>
          </>
        )}
